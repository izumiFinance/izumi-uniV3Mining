...
async function main() {
  ...
  tokenIds = await mining.getTokenIds(deployer.address);
  console.log('tokenids: ', tokenIds.map((id)=>id.toString()));

  pw = await mining.pendingReward(tokenIds[0]);
  console.log('pw: ', pw);

  pendingRewards = await mining.pendingRewards(deployer.address);
