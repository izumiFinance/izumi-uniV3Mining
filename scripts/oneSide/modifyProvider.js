...
const para = {
    ...
    rewardIdx: v[3],
    providerAddress: getAddress(v[4]),
}
...
async function main() {
  ...
  const originRewardInfo = await mining.methods.rewardInfos(0).call();
  console.log('origin rewardInfo: ', originRewardInfo);
  try {
  ...
  const currentRewardInfo = await mining.methods.rewardInfos(0).call();
  console.log('current rewardInfo: ', currentRewardInfo);
}
