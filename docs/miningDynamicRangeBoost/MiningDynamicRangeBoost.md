...
| uniFactory | address |
| swapPool | address |
| tokenStatus | mapping(uint256 => struct MiningDynamicRangeBoost.TokenStatus) |
...
```solidity
  ...
  ) external returns (address token0_, address token1_, uint24 fee_, address iziTokenAddr_, uint256 lastTouchBlock_, uint256 totalVLiquidity_, uint256 totalToken0_, uint256 totalToken1_, uint256 totalNIZI_, uint256 startBlock_, uint256 endBlock_)
```
