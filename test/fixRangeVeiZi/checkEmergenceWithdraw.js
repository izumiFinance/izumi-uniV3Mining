...
describe("test uniswap price oracle", function () {
    ...
    it("check withdraw/collec/deposit after emergencyWithdraw", async function () {
        ...
        const {delta: failLockRefund, ok: otherOk} = await emergencyWithdrawAndComputeBalanceDiff({'iZi': iZi, 'BIT': BIT, 'TOKEN0': token0, 'TOKEN1': token1}, mining, provider, '3');
        ...
        expect(failLockRefund['TOKEN0']).to.equal('0');
        expect(failLockRefund['TOKEN1']).to.equal('0');
        ...
        const {ok: ok} = await getCollectReward({'iZi': iZi, 'BIT': BIT, 'TOKEN0': token0, 'TOKEN1': token1}, mining, tester);
        ...
        const {delta: successLockRefund, ok: signerOk} = await emergencyWithdrawAndComputeBalanceDiff({'iZi': iZi, 'BIT': BIT, 'TOKEN0': token0, 'TOKEN1': token1}, mining, signer, '3');
        expect((await mining.normal())).to.equal(false);
        expect(signerOk).to.equal(true);
        expect(successLockRefund['iZi']).to.equal('0');
        expect(successLockRefund['BIT']).to.equal('0');
        expect(successLockRefund['TOKEN0']).to.equal('0');
        expect(successLockRefund['TOKEN1']).to.equal('0');
        ...
        const {reward: deltaMapOfCollect, ok: okOfCollect} = await getCollectReward({'iZi': iZi, 'BIT': BIT, 'TOKEN0': token0, 'TOKEN1': token1}, mining, tester, '3');
        for (const key in rewardPerBlockMap) {
            expect(deltaMapOfCollect[key]).to.equal('0');
        }
        expect(okOfCollect).to.equal(false);

        const {delta: deltaMapOfWithdraw, ok: okOfWithdraw} = await withdrawAndComputeBalanceDiff({'iZi': iZi, 'BIT': BIT, 'TOKEN0': token0, 'TOKEN1': token1}, mining, tester, '3');
        for (const key in rewardPerBlockMap) {
