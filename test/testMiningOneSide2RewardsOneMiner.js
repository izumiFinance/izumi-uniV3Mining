...
describe("mining one side with 2 rewards", function () {
    ...
    it("check simply mining when tokenX is tokenUni, and mining with NFT", async function () {
      ...
      var checkBlock1 = await ethers.provider.getBlockNumber();
      var tokenReward0 = BigNumber(rewardInfo.tokenPerBlock0).times(checkBlock1 - checkBlock0);
      var tokenReward1 = BigNumber(rewardInfo.tokenPerBlock1).times(checkBlock1 - checkBlock0);

      var expectAccRewardPerShare0 = muldiv(tokenReward0, q128, BigNumber(totalVLiquidity)).toFixed(0);
      var expectAccRewardPerShare1 = muldiv(tokenReward1, q128, BigNumber(totalVLiquidity)).toFixed(0);

      var accRewardPerShare0 = (await mining.accRewardPerShare0()).toString();
      var accRewardPerShare1 = (await mining.accRewardPerShare1()).toString();

      expect(accRewardPerShare0).to.equal(expectAccRewardPerShare0);
      expect(accRewardPerShare1).to.equal(expectAccRewardPerShare1);
      var expectMiner1Reward0 = muldiv(accRewardPerShare0, BigNumber(miner1VLiquidity), q128).toFixed(0);
      var expectMiner1Reward1 = muldiv(accRewardPerShare1, BigNumber(miner1VLiquidity), q128).toFixed(0);
      await checkBalance(token0, recipient2, expectMiner1Reward0);
      await checkBalance(token1, recipient2, expectMiner1Reward1);
      await checkBalance(tokenY, recipient2, miner1AmountLockY);
      // check mining info after withdraw
      expectMiningInfo = {
        amountLock: "0",
        vLiquidity: "0",
        lastTouchAccRewardPerShare0: expectAccRewardPerShare0,
        lastTouchAccRewardPerShare1: expectAccRewardPerShare1,
        uniPositionID: "0",
        isUniPositionIDExternal: true,
      };
      await checkMiningInfo(mining, "0", expectMiningInfo);
    ...
    it("check simply mining when tokenY is tokenUni, and mining with NFT", async function () {
      ...
      var mintParam = {
        ...
        tickUpper: tickRight,
        amount0Desired: "0",
        amount1Desired: miner1AmountUniY,
        amount0Min: "0",
        amount1Min: "1",
        recipient: miner1.address,
        deadline: "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
      ...
      await tokenY.transfer(miner1.address, "1000000000000000000");
      await tokenY.connect(miner1).approve(uniPositionManager.address, miner1AmountUniY);
      console.log("before uniswap mint");
      await uniPositionManager.connect(miner1).mint(mintParam);
      console.log("after uniswap mint");
      var posID = "1";
      await checkPositionOwner(uniPositionManager, posID, miner1);
      ...
      // miner1
      await tokenX.transfer(miner1.address, "1000000000000000000");
      await tokenX.connect(miner1).approve(mining.address, "40000000000000000");
      ...
      var expectTotalVLiquidity = miner1VLiquidity;
      var totalVLiquidity = (await mining.totalVLiquidity()).toString();
      expect(totalVLiquidity).to.equal(expectTotalVLiquidity);
      // check mining info after mint
      var expectMiningInfo = {
