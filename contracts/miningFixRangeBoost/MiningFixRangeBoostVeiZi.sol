...
contract MiningFixRangeBoostV2 is MiningBaseVeiZi, IERC721Receiver {
    ...
    function _newTokenStatus(uint256 nftId, TokenStatus memory newTokenStatus) internal {
        ...
        returns (uint256 vLiquidity)
    {
        ...
        _newTokenStatus(tokenId, newTokenStatus);

        emit Deposit(msg.sender, tokenId, newTokenStatus.veiZi);
    ...
    function withdraw(uint256 tokenId, bool noReward) external nonReentrant {
        ...
        totalValidVeiZi -= tokenStatus[tokenId].validVeiZi;
